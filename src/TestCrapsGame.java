...
public class TestCrapsGame
{
	 ...
	 * <code>numPlays</code> total plays.
	 */
	private int[] winerSteps = new int[100];
	...
	public TestCrapsGame()
	{
		...
		games = new Crap(new Die_1(), new Die_1()); // two "fair" Die objects
	...
	public void play(int n)
	{
		...
		{
			if (games.ifplayonegame(winerSteps, lossSteps))
			{
				numberWins++;
			}
		}
	...
	private void repoertStatesAction()
	{
		System.out.println("\nNumber of (wins,losses) for games of given length follow:\n");
		for (int i = 1; i < winerSteps.length; i++)
		{
			System.out.println("(wins,losses) ending at " + i + " roll" + ((i > 1) ? "s" : "") + ": (" + winerSteps[i] + ","
					+ lossSteps[i] + ")");
		}

		System.out.println("\nPlayed " + numberPlays + " games total.");

		System.out.println("Won " + numberWins + "/" + numberPlays + "==" + 100.0 * (0.0 + numberWins) / numberPlays + "%");
	}
	...
	{
		TestCrapsGame test = new TestCrapsGame();

		System.out.println("Starting simulation of " + NUMber_TO_PLAY + " games...");

		Crap.sOp = false; // turn off output to speed up
		// simulation
		...
		test.reportStates(); // report the output
