...
public class Crap
{
	...
	public Crap(Die_1 die_1, Die_1 die_2)
	{
		dicemal = new Dicemal(die_1, die_2);
	}

	public boolean ifplayonegame(int[] wins, int[] losses)
	{
		int steps = 1;
		...
		points = dicemal.getThe_last_Roll();

		println("First roll is: " + dicemal.getThe_last_Roll());
		...
		{
			println("Loss for player with " + points);
		...
		else // point rolled: keep going until point rerolled OR 7 rolled
		{
			...
			do
			{
				dicemal.roll();
				steps++;
				theValue = dicemal.getThe_last_Roll();
				println("Next roll is: " + theValue);
			}
			while (theValue != 7 && theValue != points);
...
//			}
			...
			else if (theValue==points)
			{
				// win: record wins and return false
				println("You win by throwing your point " + theValue);
				wins[steps]++;
				return true;
	...
	{
		winForPlayerAction(wins, steps);
		return true;
	}

	private void winForPlayerAction(int[] wins, int steps)
	{
		println("Win for player with " + points);
